...
shape_pointData <- shapefile(paste(getwd(), "/Fieldwork/Sum_UTM43N_4U_Classes.shp", sep=""))
responseCol <- "OBJECTID"

# Import polygon for masking prediction image(s)
dLower <- shapefile(paste(getwd(), "/Polygons/DL_Lower_UTM43N.shp", sep=""))
...
for (j in 1:length(s)){
  ...
  writeRaster(preds_rf2_1k, file.path(paste(getwd(), "/Veg_Classification/", sep=""), names(s[[j]])[1]), format = "GTiff", overwrite = TRUE)
}
